...
def create_model(inp_tm1, inp_t, h1_init, c1_init):
    proj = Linear([inp_tm1], [1], n_hid, random_state=random_state, name="in_proj",
                  init=forward_init)

    proj_q, proj_i, proj_emb = VqEmbedding(proj, n_hid, 512,
    ...
    r = scan(step, [proj_q], [None, h1_init, c1_init])
    out = r[0]
    hiddens = r[1]
    cells = r[2]
    ...
    """
    return pred, hiddens, cells, proj_q, proj_i, proj
...
def create_graph():
    ...
    with graph.as_default():
        ...
        pred, hiddens, cells, proj_q, proj_i, proj = r

        rec_loss = tf.reduce_mean(tf.square(pred - inputs_t))

        alpha = 1.
        beta = 0.25
        vq_loss = tf.reduce_mean(tf.square(tf.stop_gradient(proj) - proj_q))
        commit_loss = tf.reduce_mean(tf.square(proj - tf.stop_gradient(proj_q)))
