...
def create_model(inp_tm1, inp_t, h1_init, c1_init):
    def step(x_t, h1_tm1, c1_tm1):
        ...
        h1_t = s[0]
        c1_t = s[1]
        h1_q_t, h1_i_t, h1_emb = VqEmbedding(h1_t, n_hid, 8192,
                                             random_state=random_state,
                                             name="h1_vq_emb")
        c1_q_t, c1_i_t, c1_emb = VqEmbedding(c1_t, n_hid, 8192,
                                             random_state=random_state,
                                             name="c1_vq_emb")
        # not great
        h1_i_t = tf.cast(h1_i_t, tf.float32)
        c1_i_t = tf.cast(c1_i_t, tf.float32)
        return output, h1_q_t, c1_q_t, h1_t, c1_t, h1_i_t, c1_i_t

    r = scan(step, [inp_tm1], [None, h1_init, c1_init, None, None, None, None])
    out = r[0]
    q_hiddens = r[1]
    q_cells = r[2]
    hiddens = r[3]
    ...
    """
    return pred, q_hiddens, q_cells, hiddens, cells, i_hiddens, i_cells
...
def create_graph():
    ...
    with graph.as_default():
        ...
        pred, q_hiddens, q_cells, hiddens, cells, i_hiddens, i_cells = r

        rec_loss = tf.reduce_sum(tf.square(pred - inputs_t))

        alpha = 1.
        beta = 0.25
        vq_h_loss = tf.reduce_mean(tf.square(tf.stop_gradient(hiddens) - q_hiddens))
        commit_h_loss = tf.reduce_mean(tf.square(hiddens - tf.stop_gradient(q_hiddens)))

        vq_c_loss = tf.reduce_mean(tf.square(tf.stop_gradient(cells) - q_cells))
        commit_c_loss = tf.reduce_mean(tf.square(cells - tf.stop_gradient(q_cells)))
