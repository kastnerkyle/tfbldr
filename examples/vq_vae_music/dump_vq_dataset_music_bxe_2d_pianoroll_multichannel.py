...
d = np.load("music_data_jos_pianoroll_multichannel.npz")
...
with tf.Session(config=config) as sess:
    ...
    for i in range(len(image_data) // bs):
        ...
        zcn += flat_chordnames[i * bs:(i + 1) * bs]
    z = np.array(z)

chord_indices = [full_chord_lookup[cn] for cn in zcn]
...
for fi in flat_idx:
    ...
    if fi[1] == 0:
        # use self as prev at start
        left_name = list(set(nested_chordnames[fi[0]][fi[1]]))[0]
    else:
        left_name = list(set(nested_chordnames[fi[0]][fi[1] - 1]))[0]
    left = basic_chord_lookup["".join([i for i in left_name if not i.isdigit()])]

    if fi[1] == (len(nested_chordnames[fi[0]]) - 1):
        # use self at end in end
        right_name = list(set(nested_chordnames[fi[0]][fi[1]]))[0]
    else:
        right_name = list(set(nested_chordnames[fi[0]][fi[1] + 1]))[0]
    right = basic_chord_lookup["".join([i for i in right_name if not i.isdigit()])]
    chord_indices.append((left, cur, right))
    all_chord_names.append((left_name, cur_name, right_name))
...
labels = []
counter = 0
for cn in all_chord_names:
    if cn not in lcr_to_label:
        lcr_to_label[cn] = counter
        label_to_lcr[counter] = cn
        counter += 1
    lbl_i = lcr_to_label[cn]
    labels.append(lbl_i)
"""
...
np.savez("vq_vae_encoded_music_2d_pianoroll_multichannel.npz",
         z=z,
         labels=labels,
         labelnames=all_chord_names,
...
print("dumped to vq_vae_encoded_music_2d_pianoroll_multichannel.npz")
