...
d = np.load("vq_vae_encoded_music_jos_2d_pianoroll_multichannel.npz")
image_data = copy.deepcopy(d["z"])
flat_idx = d["flat_idx"]
assert len(flat_idx) == len(image_data)
...
train_conditions = conditions[:-1000]
...
def create_graph():
    ...
    with graph.as_default():
        ...
        conds = tf.placeholder(tf.float32, shape=[None, 12, 6, 1])
